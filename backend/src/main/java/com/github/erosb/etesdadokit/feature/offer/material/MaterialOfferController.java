...
public class MaterialOfferController {
    ...
    @GetMapping("{id}")
    @ApiOperation(
            value = "Get material offer by id.",
            response = MaterialOfferResponse.class
    )
    public ResponseEntity<MaterialOfferResponse> getMaterialOfferById(@PathVariable Long id) {
        return ResponseEntity.ok(materialOfferService.getById(id));
    }
